...
credit.plt <- ggplot(credits, aes(x=Condition2, y=total_credits, fill = Recipient, color = Recipient, alpha = Condition, size = Condition)) +
  ...
  theme(legend.position = 'bottom',
        ...
        axis.text.x=element_text(size = axtext, hjust = 0.8),
...
if (saveplots == 1) 
{tiff(graphname, units="in", width = plotW*2, height = plotH, res = resolution)
  print(credit.plt)
  dev.off()}
...
graphname <- paste0(path,'Reward_accept.tiff')  
...
if (saveplots == 1) 
  {tiff(graphname, units="in", width = plotW*2, height = plotH*0.75, res = resolution)
  print(bic.plt)
  dev.off()}
...
graphname <- paste0(path,'Model_iden_xp.tiff')  
...
if (saveplots == 1) {tiff(graphname, units="in", res = resolution, width = modplotW, height = plotH)
  print(xp.plot)
  dev.off()}

graphname <- paste0(path,'Model_iden_best.tiff')  
...
if (saveplots == 1) {tiff(graphname, units="in", res = resolution, width = plotW*2, height = plotH*0.75)
  print(ml.plot)
  dev.off()}
...
graphname <- paste0(path,'Effort_force.tiff')
...
graphname <- paste0(path,'Effort_reward_agent_force.tiff')
...
choice.ranef$PM_ID_choice[choice.ranef$PM_ID == 507] <- 270 # one ID mislabelled

df.map.wide <- left_join(k.ranef %>% select(!contains("Intercept")) %>% rename(PM_ID = "PM_ID_k"), 
                         df.2k.full %>% select(!contains(c("DB", "-"))) %>% select("ID", "Group_vmpfc", contains(c("_k", "_choice"))) %>% rename(PM_ID = ID), by = "PM_ID")
...
df.map.all <- df.map.wide %>% subset(Group_vmpfc != "HC") 
df.map.all <- df.map.all %>% 
  mutate(lv = scale(df.map.all$lesion)) %>% 
  mutate(rank_recipient_k_pos = scale(rank(df.map.all$Recipient_s2z1_k))) %>% 
  mutate(rank_recipient_k_neg = rank_recipient_k_pos * -1) %>% 
  mutate(rank_recipient_choice_pos = scale(rank(df.map.all$Recipient_s2z1_choice))) %>% 
  mutate(rank_recipient_choice_neg = rank_recipient_choice_pos * -1) %>% 
  mutate(rank_effort_pos = scale(rank(df.map.all$Effort_choice))) %>% 
  mutate(rank_effort_neg = rank_effort_pos * -1) %>% 
  mutate(rank_reward_pos = scale(rank(df.map.all$Reward_choice))) %>% 
  mutate(rank_reward_neg = rank_reward_pos * -1)

df.map.vmpfc <- df.map.wide %>% subset(Group_vmpfc == "vmPFC")
df.map.vmpfc <- df.map.vmpfc %>%
  mutate(lv = scale(df.map.vmpfc$lesion)) %>% 
  mutate(rank_recipient_k_pos = scale(rank(df.map.vmpfc$Recipient_s2z1_k))) %>% 
  mutate(rank_recipient_k_neg = rank_recipient_k_pos * -1) %>% 
  mutate(rank_recipient_choice_pos = scale(rank(df.map.vmpfc$Recipient_s2z1_choice))) %>% 
  mutate(rank_recipient_choice_neg = rank_recipient_choice_pos * -1) %>% 
  mutate(rank_effort_pos = scale(rank(df.map.vmpfc$Effort_choice))) %>% 
  mutate(rank_effort_neg = rank_effort_pos * -1) %>% 
  mutate(rank_reward_pos = scale(rank(df.map.vmpfc$Reward_choice))) %>% 
  mutate(rank_reward_neg = rank_reward_pos * -1)

df.map.all.k <- df.map.all %>% select("lv", starts_with("rank_recipient_k"))
if (savecsvs == 1) {write.csv(df.map.all.k, "../Lesion_analysis/all_lesion_patients/recip_k/recip_k_ranked.csv", row.names = F, na = "")}
df.map.all.choice <- df.map.all %>% select("lv", starts_with("rank_recipient_choice"))
if (savecsvs == 1) {write.csv(df.map.all.choice, "../Lesion_analysis/all_lesion_patients/recip_choice/recip_choice_ranked.csv", row.names = F, na = "")}
df.map.all.effort <- df.map.all %>% select("lv", starts_with("rank_effort"))
if (savecsvs == 1) {write.csv(df.map.all.effort, "../Lesion_analysis/all_lesion_patients/effort/effort_ranked.csv", row.names = F, na = "")}
df.map.all.reward <- df.map.all %>% select("lv", starts_with("rank_reward"))
if (savecsvs == 1) {write.csv(df.map.all.reward, "../Lesion_analysis/all_lesion_patients/reward/reward_ranked.csv", row.names = F, na = "")}

df.map.vmpfc.k <- df.map.vmpfc %>% select("lv", starts_with("rank_recipient_k"))
if (savecsvs == 1) {write.csv(df.map.vmpfc.k, "../Lesion_analysis/vmPFC_patients_only/recip_k/recip_k_ranked.csv", row.names = F, na = "")}
df.map.vmpfc.choice <- df.map.vmpfc %>% select("lv", starts_with("rank_recipient_choice"))
if (savecsvs == 1) {write.csv(df.map.vmpfc.choice, "../Lesion_analysis/vmPFC_patients_only/recip_choice/recip_choice_ranked.csv", row.names = F, na = "")}
df.map.vmpfc.effort <- df.map.vmpfc %>% select("lv", starts_with("rank_effort"))
if (savecsvs == 1) {write.csv(df.map.vmpfc.effort, "../Lesion_analysis/vmPFC_patients_only/effort/effort_ranked.csv", row.names = F, na = "")}
df.map.vmpfc.reward <- df.map.vmpfc %>% select("lv", starts_with("rank_reward"))
if (savecsvs == 1) {write.csv(df.map.vmpfc.reward, "../Lesion_analysis/vmPFC_patients_only/reward/reward_ranked.csv", row.names = F, na = "")}

# RUN LESION MAPPING
# using above csvs and see Guide_to_lesion_analysis.docx
# then extract the number of voxels / volume of damage for each patient in the regions identified in lesion mapping
# and save as csvs to load below

df.k.roi <- read_csv("data/all_patients_Recipient_k_negative.csv", show_col_types = FALSE)

df.k.roi <- df.k.roi %>% 
  mutate("PM_ID" = factor(ID))
  # add_column("PM_ID" = factor(df.map$ID), .before = "ID") 
...
plot.all.choiceV.Achoice.raw <- ggplot(df.map.patients, aes(x = choice_vox_bin, y = -1 * Recipient_s2z1_choice, fill = choice_vox_bin, colour = choice_vox_bin)) + 
  ...
  scale_x_discrete(name = "Areas 14 & 25") +
...
plot.all.kV.Rk.raw <- ggplot(df.map.patients, aes(x = k_vox_bin, y = -1 * Recipient_s2z1_k, fill = k_vox_bin, colour = k_vox_bin)) + 
  ...
  scale_x_discrete(name = "Areas 14 & 25", position = "top") +
...
```{r map-assump-all, warning=FALSE, message=FALSE, echo = FALSE, fig.height=6, fig.width=4, eval=FALSE}

m.all.choice <- lm(Recipient_s2z1_choice ~ choice_vox_bin, data = df.map.patients)
m.all.choice.rank <- lm(choice_effect_rank ~ choice_vox_bin, data = df.map.patients)

performance::check_normality(m.all.choice)
performance::check_normality(m.all.choice.rank)
performance::check_homogeneity(m.all.choice)
performance::check_homogeneity(m.all.choice.rank)

m.all.choice.check <- performance::check_model(m.all.choice)
m.all.choice.rank.check <- performance::check_model(m.all.choice.rank)

m.all.k <- lm(Recipient_s2z1_k ~ k_vox_bin, data = df.map.patients)
m.all.k.rank <- lm(k_effect_rank ~ k_vox_bin, data = df.map.patients)

m.all.k.check <- performance::check_model(m.all.k)
m.all.k.rank.check <- performance::check_model(m.all.k.rank)

performance::check_normality(m.all.k)
performance::check_normality(m.all.k.rank)
performance::check_homogeneity(m.all.k)
performance::check_homogeneity(m.all.k.rank)
...
plot.vmpfc.kV.Rk.raw <- ggplot(df.map.patients, aes(x = k_vox_bin, y = -1 * Recipient_s2z1_k, fill = k_vox_bin, colour = k_vox_bin)) + 
  ...
  scale_x_discrete(name = "Area 13", position = "top") +
...
plot.vmpfc.kV.Sk <- ggplot(df.map.patients, aes(x = k_vox_bin, y = PM_self_k, colour = k_vox_bin, fill = k_vox_bin)) + 
  ...
  scale_x_discrete(name = paste0("Area 13"), position = "top") +
  scale_y_continuous(name = paste0('*\u039A*<sub>self</sub>'), trans = "reverse") +
  scale_color_manual(values = c(agent.colours[1],agent.colours[1])) +
  scale_fill_manual(values = c(agent.fills[1],agent.fills[1])) +
  theme_classic() + 
...
```{r map-assump-vmpfc, warning=FALSE, message=FALSE, echo = FALSE, fig.height=6, fig.width=4, eval=FALSE}

m.vmpfc.choice <- lm(Recipient_s2z1_choice ~ choice_vox_bin, data = df.map.patients)
m.vmpfc.choice.rank <- lm(choice_effect_rank ~ choice_vox_bin, data = df.map.patients)

performance::check_normality(m.vmpfc.choice)
performance::check_normality(m.vmpfc.choice.rank)
performance::check_homogeneity(m.vmpfc.choice)
performance::check_homogeneity(m.vmpfc.choice.rank)

m.vmpfc.choice.check <- performance::check_model(m.vmpfc.choice)
m.vmpfc.choice.rank.check <- performance::check_model(m.vmpfc.choice.rank)

m.vmpfc.k <- lm(Recipient_s2z1_k ~ k_vox_bin, data = df.map.patients)
m.vmpfc.k.rank <- lm(k_effect_rank ~ k_vox_bin, data = df.map.patients)

m.vmpfc.k.check <- performance::check_model(m.vmpfc.k)
m.vmpfc.k.rank.check <- performance::check_model(m.vmpfc.k.rank)

performance::check_normality(m.vmpfc.k)
performance::check_normality(m.vmpfc.k.rank)
performance::check_homogeneity(m.vmpfc.k)
performance::check_homogeneity(m.vmpfc.k.rank)
...
(*p*`r pvalrf(d.means$w_pval_lc_vmPFC[d.means$measure == "Qualification"])`; Table S1).
...
(group\*recipient interaction vmPFC vs. HC odds ratio<sub>(`r r.credits.full$df_error[1]`)</sub>
(OR) [95% confidence interval]=`r format(round(r.credits$Coefficient[r.credits$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`
...
vmPFC vs. LC
OR<sub>(`r r.credits.full$df_error[1]`)</sub>=`r format(round(r.credits$Coefficient[r.credits$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`
...
*p*`r pvalrf(credits.con.all$p.value[credits.con.all$contrast == "other vmPFC / other LC"])`). 
...
SE=`r format(round(credits.con.all$SE[credits.con.all$contrast == "self vmPFC / self LC"], digits=2), nsmall=2)`,
Z=`r format(round(credits.con.all$z.ratio[credits.con.all$contrast == "self vmPFC / self LC"], digits=2), nsmall=2)`,
...
SE=`r format(round(credits.con.all$SE[credits.con.all$contrast == "self vmPFC / self HC"], digits=2), nsmall=2)`,
Z=`r format(round(credits.con.all$z.ratio[credits.con.all$contrast == "self vmPFC / self HC"], digits=2), nsmall=2)`,
...
graphname <- paste0(path,'Figure_2.tiff')

if (savefullplots == 1) 
  {tiff(graphname, units="in", width = 12, height = 7.5, res = resolution)
  print(task.plt)
  dev.off()}
...
*p*`r pvalrf(choice.con.all$p.value[choice.con.all$contrast == "other vmPFC / other LC"])`; Figure 3B). 
...
graphname <- paste0(path,'Figure_3.tiff')

if (savefullplots == 1) 
  {tiff(graphname, units="in", width = 8, height = 10, res = resolution)
  print(behav.plt)
  dev.off()}
...
(*p*s<`r format(round(max(credits.con.all$p.value[credits.con.all$contrast == "other vmPFC / other HC"], credits.con.all$p.value[credits.con.all$contrast == "other vmPFC / other LC"], choice.con.all$p.value[choice.con.all$contrast == "other vmPFC / other HC"], choice.con.all$p.value[choice.con.all$contrast == "other vmPFC / other LC"]), digits=2), nsmall=2)`; see Results).
...
(*p*s>`r format(round(min(credits.con.all$p.value[credits.con.all$contrast == "self vmPFC / self HC"], credits.con.all$p.value[credits.con.all$contrast == "self vmPFC / self LC"], choice.con.all$p.value[choice.con.all$contrast == "self vmPFC / self HC"], choice.con.all$p.value[choice.con.all$contrast == "self vmPFC / self LC"]), digits=2), nsmall=2)`) and 
...
`r format(round(min(choices.bf01.hc.vmpfc.self$bf, choices.bf01.lc.vmpfc.self$bf,  credits.bf01.lc.vmpfc.self$bf), digits=2), nsmall=2)`).
...
*p*`r pvalrf(r.choice$p[r.choice$Parameter == "Reward"])`; Figure 3D)), particularly when the reward was for themselves compared to another person 
...
from choice consistency (inverse stochasticity `r "<i>\u03B2</i>"` parameter) and therefore allow us to test which of these parameters differ 
...
The robustness of the model comparison procedure and winning model was established using simulated data (see Methods). Model identifiability results demonstrated perfect identifiability (Figure 4A). The simulations also showed very good recovery of all parameters in the 2`r "<i>\u039A</i>"`2`r "<i>\u03B2</i>"` winning model
...
graphname <- paste0(path,'Figure_4.tiff')

if (savefullplots == 1) 
  {tiff(graphname, units="in", width = 12, height = 10, res = resolution)
  print(model.plot)
  dev.off()}
...
`r "<i>\u039A</i>"` parameters differed between groups with a  GLMM (see Methods).
Damage to vmPFC decreased prosociality compared to both control groups 
(group\*recipient interaction vmPFC vs. LC
OR=`r format(round(r.k$Coefficient[r.k$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`
...
vmPFC vs. HC
OR=`r format(round(r.k$Coefficient[r.k$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`
...
other ratio=`r format(round(k.con.all$ratio[k.con.all$contrast == "other vmPFC / other LC"], digits=2), nsmall=2)`,
...
self ratio=`r format(round(k.con.all$ratio[k.con.all$contrast == "self vmPFC / self LC"], digits=2), nsmall=2)`,
...
*p*`r pvalrf(k.con.all$p.value[k.con.all$contrast == "(self / other HC) / (self / other LC)"])`) further supporting the specificity of vmPFC damage. Finally, all groups showed higher effort discounting of rewards for other compared to self (Figure 4D) indicating that the bias towards helping oneself, occurs across healthy controls and patients, regardless of lesion location. 

We used the same approach to examine whether vmPFC damage affected
decision consistency (inverse stochasticity `r "<i>\u03B2</i>"`
...
(*p*s<`r pvalr(max(beta.con.ag$p.value[beta.con.ag$contrast == "self vmPFC - other vmPFC"], beta.con.ag$p.value[beta.con.ag$contrast == "self LC - other LC"], beta.con.ag$p.value[beta.con.ag$contrast == "self HC - other HC"]))`; Figure S1). 
...
*p*s>`r format(round(min(r.beta$p[r.beta$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], r.beta$p[r.beta$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"]), digits=2), nsmall=2)`) or that overall levels of decision noise differed between groups
*p*s>`r format(round(min(r.beta$p[r.beta$Parameter == "Group (vmPFC vs. LC)"], r.beta$p[r.beta$Parameter == "Group (vmPFC vs. HC)"]), digits=2), nsmall=2)`; Table S5).
...
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=3), nsmall=3)`
...
*p*`r pvalrf(r.force1$p[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"])`; Figure 5), 
particularly at higher effort levels (group\*recipient\*effort
interaction
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=3), nsmall=3)`
...
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. HC)"], digits=3), nsmall=3)`
...
b<sub>(`r r.success.full$df_error[1]`)</sub>=`r format(round(r.success$Coefficient[r.success$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=3), nsmall=3)`
...
graphname <- paste0(path,'Figure_5.tiff')

if (savefullplots == 1) 
  {tiff(graphname, units="in", width = 9.5, height = 4, res = resolution)
  print(force.e.plt)
  dev.off()}
...
LMM group\*recipient interaction 
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`
...
contrast=`r format(round(force1.con.all$estimate[force1.con.all$contrast == "self LC - other LC"], digits=2), nsmall=2)`,
SE=`r format(round(force1.con.all$SE[force1.con.all$contrast == "self LC - other LC"], digits=2), nsmall=2)`,
Z=`r format(round(force1.con.all$z.ratio[force1.con.all$contrast == "self LC - other LC"], digits=2), nsmall=2)`,
*p*`r pvalrf(force1.con.all$p.value[force1.con.all$contrast == "self LC - other LC"])`;
LMM group\*recipient interaction 
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`
[`r format(round(r.force1$CI_low[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`,
`r format(round(r.force1$CI_high[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`],
*p*`r pvalrf(r.force1$p[r.force1$Parameter == "Recipient (Self vs. Other) * Group (vmPFC vs. LC)"])`).
...
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`
...
b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`
...
graphname <- paste0(path,'Figure_6.tiff')

if (savefullplots == 1) 
  {tiff(graphname, units="in", width = 9, height = 6, res = resolution)
  print(map.roi.allp.plt)
  dev.off()}
...
graphname <- paste0(path,'Figure_7.tiff')
...
[`r format(round(r.force1$CI_low[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`,
`r format(round(r.force1$CI_high[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. HC)"], digits=2), nsmall=2)`],
*p*`r pvalrf(r.force1$p[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. HC)"])`; Table S5). 
The corresponding interaction for vmPFC patients compared to lesion patients was not significant
(b<sub>(`r r.force1.full$df_error[1]`)</sub>=`r format(round(r.force1$Coefficient[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`
[`r format(round(r.force1$CI_low[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`,
`r format(round(r.force1$CI_high[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. LC)"], digits=2), nsmall=2)`],
*p*`r pvalrf(r.force1$p[r.force1$Parameter == "Effort * Recipient (Self vs. Other) * Reward * Group (vmPFC vs. LC)"])`.
...
```{r lesion-size-plot, echo = FALSE, warning=FALSE, message=FALSE, fig.height=10, fig.width=9.5}
...
lesion.size.plt <- plot_grid(lesion.size.plt, lesion.size.leg, nrow=2, rel_widths = c(1,0.25), rel_heights = c(1,0.15))
...
```{r credit-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
...
```{r choice-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
...
```{r k-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
...
```{r b-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
...
```{r force-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
...
```{r success-posthoc-f, echo = FALSE, warning=FALSE, eval=FALSE}
